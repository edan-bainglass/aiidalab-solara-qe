...
class ElementPortal:
    ...
    def use_portal(self) -> List[Element]:
        portal_elements, set_portal_elements = solara.use_state(
            cast(PortalElements, {})
        )
...
def TesterLayout(
    ...
    show_app_bar = (
        (title and (len(routes) > 1 and navigation))
        or bool(children_appbar)
        or bool(use_drawer)
        or bool(children_appbartitle)
        or bool(tabs_element)
    )
    ...
    if (tabs_element is None) and routes and navigation and (len(routes) > 1):
        with solara.lab.Tabs(
            value=index, on_value=set_path, align="center"
        ) as tabs_element:
            for route in routes:
                ...
                with v.AppBar(color=color, dark=toolbar_dark, v_slots=v_slots):
                    if use_drawer:
                        ...
                        with v.Menu(
                            ...
                            pass
                            v.Sheet(
                                children=children_sidebar,
                                style_="padding: 12px; min-width: 400px",
                            )
                    ...
                    for i, child in enumerate(children_appbar):
                        ...
                        if child._key is None:
                            solara.display(
                                child.key(f"app-layout-appbar-user-child-{i}")
                            )
                        else:
                            solara.display(child)
                    solara.Button(
                        icon_name="mdi-fullscreen",
                        on_click=lambda: set_fullscreen(True),
                        icon=True,
                        dark=False,
                    )
                ...
                if use_drawer:
                    with v.NavigationDrawer(
                        ...
                        v.Html(
                            tag="div",
                            children=children_sidebar,
                            style_="padding: 12px; height: 100%",
                        ).meta(ref="sidebar-content")
            if show_app_bar:
                ...
                with v.AppBar(
                    ...
                    if fullscreen:
                        solara.Button(
                            icon_name="mdi-fullscreen-exit",
                            on_click=lambda: set_fullscreen(False),
                            icon=True,
                            dark=False,
                        )
            # in vue2 is was v-content, in vue3 it is v-main
            MainComponent = (
                v.Main if solara.util.ipyvuetify_major_version == 3 else v.Content
            )  # type: ignore
            with MainComponent(
                class_="solara-content-main", style_="height: 100%;"
            ).key("app-layout-content"):
